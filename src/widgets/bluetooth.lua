...
return function()
    ...
    {
      {
        {
          id = "icon",
          image = gears.color.recolor_image(icondir .. "bluetooth-off.svg", color["Blue200"]),
  ...
  awful.widget.watch(
    ...
    function(_, stdout)
      ...
      bluetooth_widget.icon_margin.icon_layout.icon:set_image(gears.color.recolor_image(icon .. ".svg", color["Blue200"]))
    end,
    bluetooth_widget
  )

  -- Signals
  Hover_signal(bluetooth_widget, "#1e1e1edd", color["Blue200"])
