...
return function()
        ...
        {
          {
            {
              id = "icon",
              image = gears.color.recolor_image(icondir .. "battery-unknown.svg", color["Purple200"]),
      ...
      function(stdout)
        ...
        if status == 'fully-charged' or status == 'charging' and battery_percentage == 100 then
          ...
          battery_widget.container.battery_layout.icon_margin.icon_layout.icon:set_image(gears.surface.load_uncached(
            gears.color.recolor_image(icondir .. icon .. '.svg', color["Purple200"])))
          return
        end
  ...
  Hover_signal(battery_widget, "#1e1e1edd", color["Purple200"])
