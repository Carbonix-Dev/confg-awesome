...
return function()
        ...
        {
          {
            {
              id = 'icon',
              image = gears.color.recolor_image(icondir .. "no-internet" .. ".svg", color["Red200"]),
  ...
  local check_for_internet = [=[
        net_uptime_server=]=] .. user_vars.uptime_server .. [=[
        status_ping=0
        packets="$(ping -q -w2 -c2 $net_uptime_server | grep -o "100% packet loss")"
        if [ ! -z "${packets}" ];
        then
            status_ping=0
        else
  ...
  local update_wireless = function()
    ...
    local notify_connected = function(essid)
      ...
      -- local app_name = "Settings | Network"
      -- local icon = icondir .. "wifi-strength-" .. tostring(wifi_strength_rounded) .. ".svg"
    ...
    local update_wireless_icon = function(strength)
      awful.spawn.easy_async_with_shell(
        ...
        function(stdout)
          ...
          network_widget.container.network_layout.icon_margin.icon_layout.icon:set_image(gears.color.recolor_image(icondir .. icon .. ".svg", color["Red200"]))
        end
      )
    end
  ...
  -- Signals
  Hover_signal(network_widget, '#1e1e1edd', color["Red200"])
