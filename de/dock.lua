...
return function(screen, programs) --, widgets)
  ...
  local dock = awful.popup {
    ...
    screen = screen,
    type = "dock",
    height = user_vars.dock_icon_size + 5,
    placement = function(c) awful.placement.bottom_left(c, { margins = dpi(3) }) end,
    shape = function(cr, width, height)
      gears.shape.rounded_rect(cr, width, height, 5)
    end
  }
  ...
  local fakedock = awful.popup {
    ...
    id = "fakedock",
    height = dpi(10),
    placement = function(c) awful.placement.bottom_left(c, { margins = dpi(0) }) end,
  }
  ...
  local function create_incicator_widget(prog)
    ...
    for index, pr in ipairs(prog) do
      local indicators = { layout = wibox.layout.flex.horizontal, spacing = dpi(2.5) }
